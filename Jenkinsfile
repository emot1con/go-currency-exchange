                    ...
                    "Integration Test": {
                        ...
                        script {
                            try {
                                ...
                                sh "docker network create test-network-currency-service-${env.BUILD_NUMBER} || true"
                                ...
                                docker run -d --name currency-exchange-test-${env.BUILD_NUMBER} \
                                    --network test-network-currency-service-${env.BUILD_NUMBER} \
                                    -p 8080:8080 \
                                ...
                                sh """
                                timeout 60s bash -c 'while ! docker run --rm --network test-network-currency-service-${env.BUILD_NUMBER} curlimages/curl:latest curl -f http://currency-exchange-test-${env.BUILD_NUMBER}:8080/health; do sleep 2; done' || {
                                ...
                                echo "Running integration test..."
                                sh """
                                docker run --rm --network test-network-currency-service-${env.BUILD_NUMBER} \
                                    -v \$(pwd):/workspace \
                                    -w /workspace \
                                ...
                                docker network rm test-network-currency-service-${env.BUILD_NUMBER} || true
